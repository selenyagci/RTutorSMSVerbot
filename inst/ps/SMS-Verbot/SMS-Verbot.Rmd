...
Quiz: Was denken Sie, um wie viel Prozent ist die Zahl der durch Ablenkung verursachten Unfälle in den ersten zehn Monaten des Jahres 2022 im Vergleich zum Vorjahr (2021) gestiegen?
...
Quiz: Welche der folgenden Aussagen ist korrekt, wenn `filter(state == 53)` auf den Datensatz data angewendet wird?

(1) Die Zeilen, in denen state den Wert 53 hat, werden im Ergebnis beibehalten.
(2) Alle Spalten, die nicht state heißen, werden entfernt.
(3) Es werden neue Zeilen mit state = 53 hinzugefügt.

```{r eval=FALSE}
# Run line to answer the quiz above
answer.quiz("dplyr-Paket")
```
...
Quiz: In wie viel Prozent der Fälle haben wir Informationen zu allen Variablen einer Beobachtung?

Answer: 

```{r eval=FALSE}
# Run line to answer the quiz above
answer.quiz("Anteil Beobachtungen ohne NAs an Gesamten")
```
...
Quiz: Welcher der folgenden Bundesstaaten gehört nicht zur Behandlungsgruppe?
...
```{r eval=FALSE}
# Run line to answer the quiz above
answer.quiz("Behandlungs- und Kontrollgruppe der US-Bundesstaaten")
```
...
Quiz: In wie vielen Bundesstaaten wird das SMS-Verbot als "stark" eingestuft?

Answer: 

```{r eval=FALSE}
# Run line to answer the quiz above
answer.quiz("Bundesstaaten mit starken SMS-Verboten")
```
...
Quiz: Wie hoch ist die durchschnittliche monatliche Arbeitslosenquote in der Kontrollgruppe?
...
Quiz: Welche der folgenden Interpretationen des DiD-Schätzwerts von $0,574$ ist korrekt?

(1) Im Vergleich zur Kontrollgruppe führte das SMS-Verbot in der Behandlungsgruppe zu einer Reduktion der tödlichen Verkehrsunfälle mit einem einzelnen Fahrzeug und einem einzelnen Insassen um durchschnittlich 0,574 Fällen pro Monat, ohne Berücksichtigung des Zeitpunkts der Einführung.
(2) Im Vergleich zur Kontrollgruppe (den US-Bundesstaaten ohne SMS-Verbot) stieg die durchschnittliche Anzahl tödlicher Verkehrsunfälle mit einem einzelnen Fahrzeug und einem einzelnen Insassen pro Monat in den US-Bundesstaaten mit SMS-Verbot (Behandlungsgruppe) nach Einführung des SMS-Verbots (Experimentalphase) um 0,574 mehr als im Zeitraum vor der Einführung des Verbots (Vorbehandlungsphase).
(3) Nach Einführung des SMS-Verbots stieg die durchschnittliche Anzahl tödlicher Verkehrsunfälle mit einem einzelnen Fahrzeug und einem einzelnen Insassen pro Monat um genau 0,574 in allen US-Bundesstaaten, unabhängig davon, ob sie ein SMS-Verbot eingeführt hatten oder nicht.
(4) Im Vergleich zur Kontrollgruppe (den US-Bundesstaaten ohne SMS-Verbot) sank die durchschnittliche Anzahl tödlicher Verkehrsunfälle mit einem einzelnen Fahrzeug und einem einzelnen Insassen pro Monat in den US-Bundesstaaten mit SMS-Verbot (Behandlungsgruppe) nach Einführung des SMS-Verbots (Experimentalphase) um 0,574 mehr als im Zeitraum vor der Einführung des Verbots (Vorbehandlungsphase).

```{r eval=FALSE}
# Run line to answer the quiz above
answer.quiz("Manueller DiD-Ansatz Interpretation")
```
...
Quiz: Welcher Koeffizient ist der DiD-Schätzer in unserem Regressionsmodell (4.1)?
...
```{r eval=FALSE}
# Run line to answer the quiz above
answer.quiz("Koeffizient des DiD-Schätzers")
```
...
Quiz: Wofür könnte der konstante Term $ß_0$ = $18,011$ in unserem Regressionsmodell stehen?

(1) Die durchschnittliche Anzahl tödlicher Verkehrsunfälle mit einem einzelnen Fahrzeug und einem einzelnen Insassen in der Kontrollgruppe.
(2) Die durchschnittliche Anzahl tödlicher Verkehrsunfälle mit einem einzelnen Fahrzeug und einem einzelnen Insassen in der Behandlungsgruppe während der Vorbehandlungsphase.
(3) Die durchschnittliche Anzahl tödlicher Verkehrsunfälle mit einem einzelnen Fahrzeug und einem einzelnen Insassen in der Kontrollgruppe während der Vorbehandlungsphase.
(4) Die durchschnittliche Anzahl tödlicher Verkehrsunfälle mit einem einzelnen Fahrzeug und einem einzelnen Insassen in der Kontrollgruppe während der Experimentalphase.

```{r eval=FALSE}
# Run line to answer the quiz above
answer.quiz("Koeffizient ß_0")
```
...
Quiz: Wofür könnte der Koeffizient $ß_2$ = $6,611$ in unserem Regressionsmodell stehen?

(1) Die Differenz in der durchschnittlichen Anzahl tödlicher Verkehrsunfälle mit einem einzelnen Fahrzeug und einem einzelnen Insassen zwischen Behandlungs- und Kontrollgruppe während der Experimentalphase.
(2) Die Differenz in der durchschnittlichen Anzahl tödlicher Verkehrsunfälle mit einem einzelnen Fahrzeug und einem einzelnen Insassen zwischen Behandlungs- und Kontrollgruppe während der Vorbehandlungsphase.
(3) Die Differenz in der durchschnittlichen Anzahl tödlicher Verkehrsunfälle mit einem einzelnen Fahrzeug und einem einzelnen Insassen zwischen Behandlungs- und Kontrollgruppe während der Vorbehandlungs- und der Experimentalphase.

```{r eval=FALSE}
# Run line to answer the quiz above
answer.quiz("Koeffizient ß_2")
```
...
Quiz: Für welche Variable(n) aus der vorherigen Regression lm(accidentsvso ~ treat_exp + treated + exp, data = did_data) sollten Ihrer Meinung nach Fixeffekte auf Bundesstaatsebene und Monatsebene hinzugefügt werden?

(1) treat_exp
(2) exp
(3) treated und exp
(4) Keine Variable muss entfernt werden

```{r eval=FALSE}
# Run line to answer the quiz above
answer.quiz("Fixeffekte")
```
...
Quiz: Betrachten Sie die Ergebnisse der drei Regressionen. Wie verändert sich der geschätzte DiD-Koeffizient (`treat_exp`), wenn in den Modellen Fixeffekte hinzugefügt werden?
...
```{r eval=FALSE}
# Run line to answer the quiz above
answer.quiz("Vergleich der Regressionen")
```
...
Quiz: Vergleichen Sie die Ergebnisse der dritten und vierten Regression. Wie verändert sich der Standardfehler des DiD-Schätzers, wenn in der vierten Regression cluster-robuste Standardfehler verwendet werden?
...
```{r eval=FALSE}
# Run line to answer the quiz above
answer.quiz("Cluster-robuste Standardfehler")
```
...
Quiz: Was könnten Sie sich unter dem Staggered Difference-in-Differences-Ansatz vorstellen? Der Ansatz geht davon aus, dass eine Intervention zu unterschiedlichen Zeitpunkten eingeführt wird
...
```{r eval=FALSE}
# Run line to answer the quiz above
answer.quiz("Staggered Difference-in-Differences-Ansatz")
```
...
Quiz: Welche der folgenden Interpretationen des Schätzwerts von $-0,037$ ist korrekt?

(1) Im Vergleich zur Kontrollgruppe führte das SMS-Verbot in den Bundesstaaten, die das Verbot eingeführt haben, zu einer Reduktion der Verkehrsunfälle mit einem einzelnen Fahrzeug und einem einzelnen Insassen um 3,7 Prozent.
(2) Im Vergleich zur Kontrollgruppe führte das SMS-Verbot in den Bundesstaaten, die das Verbot eingeführt haben, zu einer Reduktion der Verkehrsunfälle mit einem einzelnen Fahrzeug und einem einzelnen Insassen um 0,037 Prozent.
(3) Im Vergleich zur Kontrollgruppe führte das SMS-Verbot in den Bundesstaaten, die das Verbot eingeführt haben, zu einer Erhöhung der Verkehrsunfälle mit einem einzelnen Fahrzeug und einem einzelnen Insassen um 3,7 Prozent.
(4) Im Vergleich zur Kontrollgruppe führte das SMS-Verbot in den Bundesstaaten, die das Verbot eingeführt haben, zu einer Erhöhung der Verkehrsunfälle mit einem einzelnen Fahrzeug und einem einzelnen Insassen um 0,037 Prozent.

```{r eval=FALSE}
# Run line to answer the quiz above
answer.quiz("Interpretation von -0,037")
```
...
Quiz: Welche Variable(n) aus der vorherigen Regression `lm_robust(laccidentsvso2 ~ txmsban + lpop + lunemp + permale2 + lrgastax, data...)` werden durch die Variablen `strongban` und `weakban` ersetzt?

(1) txmsban
(2) txmsban und lpop
(3) Keine der Variablen wird ersetzt, sie werden nur ergänzt.
(4) lpop

```{r eval=FALSE}
# Run line to answer the quiz above
answer.quiz("Variablen strongban und weakban")
```
...
Quiz: Für die nächste Modellspezifikation möchten wir die abhängige Variable `laccident2` aus der vorherigen Modellspezifikation durch eine andere Variable ersetzen. In der neuen Modellspezifikation repräsentiert die abhängige Variable den logarithmierten Wert ($+ 1$) der Anzahl tödlicher Verkehrsunfälle, an denen mehrere Fahrzeuge oder Insassen beteiligt sind. Wie lautet der Name dieser abhängigen Variable?

Answer: 

```{r eval=FALSE}
# Run line to answer the quiz above
answer.quiz("Abhängige Variable der dritten Modellspezifikation")
```
...
```{r "7_12",warning=FALSE, message=FALSE}
library("MASS")
...
Quiz: Mit welchem Befehl können wir nur die Beobachtungen aus dem Datensatz `data` auswählen, die bis einschließlich des Jahres 2009 vorliegen?

(1) data = data %>% filter(year <= 2009)
(2) data = data %>% filter(year < 2009)
(3) data = data %>% filter(year == 2009)
(4) data = data %>% filter(year >= 2010)

```{r eval=FALSE}
# Run line to answer the quiz above
answer.quiz("Auswählen der Daten bis einschließlich 2009")
```
...
```{r "7_14",warning=FALSE, message=FALSE}
library("dplyr")
library("estimatr")
...
```{r "7_16",warning=FALSE, message=FALSE}
library("dplyr")
...
Quiz: Welche der folgenden Aussagen beschreibt ein **starkes** SMS-Verbot?

(1) Ein Verbot, das nur junge Fahrer*innen gilt und sekundär durchgesetzt wird.
(2) Ein Verbot, das universell angewendet und primär durchgesetzt wird.
(3) Ein Verbot, das lediglich in Schulen und auf Schulwegen gilt.
(4) Ein Verbot, das freiwillig von Fahrern beachtet wird.

```{r eval=FALSE}
# Run line to answer the quiz above
answer.quiz("Wiederholung der Klassifizierung der Verbote")
```
...
```{r "8_6",fig.height=8, fig.width=6}
library("ggplot2")
library("patchwork")
...
```{r "8_9",fig.height=8, fig.width=6}
library("ggplot2")
library("patchwork")
...
```{r "8_12",fig.height=8, fig.width=6}
library("ggplot2")
library("patchwork")
...
```{r "8_15",fig.height=8, fig.width=6}
library("ggplot2")
library("patchwork")
...
Quiz: Wie hoch ist der relative Rückgang der Unfälle im ersten Monat nach Einführung eines starken Verbots?

(1) 12 %
(2) 38 %
(3) 0 %
(4) Es gab einen relativen Anstieg der Unfälle.

```{r eval=FALSE}
# Run line to answer the quiz above
answer.quiz("Abbildung 7.4")
```
